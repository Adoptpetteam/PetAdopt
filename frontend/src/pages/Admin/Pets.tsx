...
const navigate = useNavigate();
  const { data: pets, isLoading, error } = useListPet({ resource: "pets" });
  ...
  if (!pets || pets.length === 0) {
    return (
      <div className="max-w-[1100px] mx-auto py-10 px-6">
        ...
        <p className="text-center text-gray-500">Chưa có thú cưng nào. Hãy thêm thú cưng mới!</p>
        <button
          onClick={() => navigate("/admin/pets/add")}
          className="mt-4 px-6 py-3 bg-[#6272B6] text-white rounded-full"
        >
          + Thêm thú cưng mới
        </button>
      </div>
          ...
          >
            <div className="flex gap-4 items-center">
              ...
              <div>
                ...
                <p className="text-sm">
                  {p.color} | {p.vaccinated ? "Đã tiêm" : "Chưa tiêm"} | {p.neutered ? "Đã triệt sản" : "Chưa"}
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button
                onClick={() => navigate(`/admin/pets/edit/${p._id}`)}
