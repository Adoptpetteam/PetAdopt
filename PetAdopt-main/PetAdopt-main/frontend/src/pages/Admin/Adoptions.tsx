...
export default function Adoptions() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-[#6272B6] mb-6">Danh sách đơn nhận nuôi</h1>
          ...
          <tbody>
            {orders.map((o: any) => (
              <tr key={o.id} className="border-t">
                ...
                <td className="p-4">{o.donationType === "monthly" ? "Hàng tháng" : o.donationType === "yearly" ? "Cả năm" : "-"}</td>
                <td className="p-4">{o.certificateType === "digital" ? "Điện tử" : o.certificateType === "physical" ? "Bản cứng" : "-"}</td>
                <td className="p-4">
                  <span className="px-3 py-1 rounded-full bg-yellow-100 text-yellow-600 ">{o.status}</span>
                </td>
                <td className="p-4 flex gap-2">
                  <button onClick={() => navigate(`/admin/pet/${o.petId}`)} className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">
                    Xem thú cưng
                  </button>

                  <button onClick={() => deleteOrder(o.id)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">
                    Xóa
                  </button>
                </td>
              </tr>
